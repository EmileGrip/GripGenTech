...
const EmployeeGapsChartArea = ({ select, data }) => {
  ...
  const employeeGapsDataByPercentage = {
    labels: data?.labels,
    datasets: [
      {
        fill: true,
  ...
  useEffect(() => {
    if (select === "select__absolute") {
      setDatasetValue(employeeGapsData);
    } else {
      setDatasetValue(employeeGapsDataByPercentage);
    }
  }, [select, data]);
...
export default EmployeeGapsChartArea;
